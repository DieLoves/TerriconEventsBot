...
- Материализовать получателей необработанных domain changes в delivery-записи с уникальными
  ключами, используя текущие подписки и объединение старых/новых категорий. Отмечать change
  обработанным даже при отсутствии получателей, чтобы будущие подписчики не получили старое
  уведомление.
...
- Отправлять большие дайджесты несколькими Telegram-сообщениями, сохраняя единый batch ID и
  прогресс подтверждённых частей для продолжения после рестарта.
...
Статус: завершён 2026-09-19.

- Для restart-safe broadcast delivery хранить lease и прогресс подтверждённых Telegram-сообщений
  в PostgreSQL; исходной схеме этапа 2 этих полей недостаточно.
...
Статус: выполняется. Runtime, scheduler, lifecycle, heartbeat, healthcheck и Docker Compose
подготовлены и проверены 2026-09-19; host backup/restore и production rollout ещё не выполнены.
